...
<?php 
  ...
  /* Dates */
  if ($row->drealty_listing_inactive_date == null) 
    {$solddate = $row->changed;}
      else
    {$solddate = $row->drealty_listing_inactive_date; }   
  #$solddate = $row->drealty_listing_inactive_date; 
  echo "<!-- SOLDDATE 1 ".  $solddate ." -->";

  $unix = $solddate ;
  $solddate  = gmdate("M Y", $unix);
    echo "<!-- SOLDDATE 2 ".  $solddate ." -->";
  $today = time();
  $four_months = $unix + (120 * 24 * 60 * 60);// Four months from sold date
  $unix_date = $unix;
  $statusBadge = NULL; // the red badge for Sold, Exclusive and Waterfront.
  ...
  if ($display == 1){
      ...
      if ($status == "ACTIVE" || $status =="Active")          { $statusBadge = "";}
  
  if ($waterfront == "Yes" && $dReatly_status == 1) { $statusBadge = "<span class=\"exclusive\">waterfront!</span>";}
  if ($status == "Exclusive" && $waterfront == "Yes") { $statusBadge = "<span class=\"exclusive\">waterfront exclusive!</span>";}
  
  if ($status == "Conditional Sale" && $dReatly_status == 1){ 
    $statusBadge = "<span class=\"exclusive\">conditional sale!</span>"; 
    $sold = 1;  
  }
  if ($status == "Conditional Sale" && $dReatly_status == 0){ 
    $statusBadge = "<span class=\"exclusive\">sold!</span>"; 
    $sold = 1;  
  }
  if ($status == "CONDITIONAL SALE" && $dReatly_status == 1){ 
    $statusBadge = "<span class=\"exclusive\">conditional sale!</span>"; 
  ...
  if ($status == "Sold / Leased")   { $statusBadge = "<span class=\"exclusive\">sold!</span>"; $sold = 1;  }  
  if ($status == "SOLD")            { $statusBadge = "<span class=\"exclusive\">sold!</span>"; $sold = 1;  }
  if ($status == "Exclusive")       { $statusBadge = "<span class=\"exclusive\">exclusive!</span>";}
